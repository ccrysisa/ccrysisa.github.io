...
deepin 20.9 预装的系统库比较陈旧（沿袭自“侏罗纪”时代的 Debian），特别是 `libstdc++`，如果你想使用 LLVM 的产品，预装的版本根本不满足，需要手动安装较新版本的包（`libstd++-12-dev` 及以上的版本）:
...
deepin 20.9 没法直接使用 `su` 命令，而是需要使用 `sudo su` 命令来进入 root 用户。

#### 个性化

本人的配置: [壁纸](https://haowallpaper.com/homeViewLook/16358558967713152) | [图标](https://www.gnome-look.org/p/1340791)
...
{{< admonition tip >}}
如果是笔记本的小尺寸屏幕，在 tweaks 里面字体 (font) 设置处将 font size 缩放至 1.25 即可；或者在显示 (Display) 设置中开启分数缩放，调成至 125% 即可。(但是非整数缩放在 GNOME 48 才正式支持)
{{< /admonition >}}
...
deepin 20.9 默认没有预装完整的 man 手册，需要手动安装:
...
~~使用项目 clash-for-linux-backup (目前已删库) 来配置 Linux 发行版的网络代理。~~
使用项目 [clash-verge-rev](https://github.com/clash-verge-rev/clash-verge-rev) 来配置 Linux 发行版的网络代理。在 GLW 内可以使用 [gitclone](http://gitclone.com) 镜像站来加快 clone 的速度。
...
可以在 `~/.bashrc` 或 `~/.bash_aliases` 文件中设置命令 / 脚本的别名。

如果你没有关闭 clash-for-linux 服务就关机，那么重启后可能会出现，输入密码无法进入图形界面重新返回登录界面，这一循环状况。这个是有些 Linux 发行版默认的 shell 是 dash，但位于 `/etc/` 路径下的 clash 服务脚本需要使用 bash 才能运行造成的，有几种方式可以解决该问题。
...
如果你已经处于无限登录界面循环这一状况，可以通过 `Ctrl + Alt + [F1-F12]` 切换进入 TTY/GUI 界面进行修改。

{{< /admonition >}}

透明代理可采用 [dae](https://github.com/daeuniverse/dae) 这一开源方案来实现。现在因为 [v2rayA](https://github.com/v2rayA/v2raywA) 支持的协议比较老旧，导致上网体验不佳，所以不再推荐其作为透明代理的方案。
...
在 Linux 上如果默认的非等宽字体 (例如 UI 显示) 不尽人意，可以使用 [Sarasa Gothic (更纱黑体 / 更紗黑體)](https://github.com/be5invis/Sarasa-Gothic) 这一方案。
...
通过 `update-grub` 命令可以更新 grub2 里的系统引导项。

...
> **_指標，不僅是 C 語言的靈魂，更是儲存-執行模型中不可或缺的存在。_**
...
`void *` 只能表示地址，而不能对所指向的地址区域的内容进行操作。因为通过 `void *` 无法知道所指向区域的 size，所以无法对区域的内容进行操作，必须对 `void *` 进行 **_显示转换_** 才能操作指向的内容以及对指针自身进行递增或递减操作。在 GNU C 中，`void *` 除了并不能使用 `*` 解引用运算之外，其它操作均与 `char *` 相同，包括 `++`, `--`, `+=` 等操作。
...
理解 C 语言中的 Function 以及 Function Pointer 的核心在于理解 **_Function Designator_** 这个概念，函数名字必然是 Function Designator，其它的 designator 则是根据以下两条规则进行推导得来。
{{< /admonition >}}

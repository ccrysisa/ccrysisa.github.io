...
Peterson 算法相比之前的方案只是将顺序的设定提前了，但却解决了之前方案的互斥问题，因为这样设定会使得 `your flag is raised and turn is your_id` 这个状态只可能出现在对方处于 CS 时，如果对方同时与自己争夺 CS 的进入权时，对方的状态是 `your flag is raised and turn is my_id`，从而将争夺进入权和已处于 CS 的 **_状态_** 进行了区分，解决了互斥问题。
